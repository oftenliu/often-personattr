...
def import_Market1501Attribute(dataset_dir):
    ...
    test_attribute = {}
    train_attribute = {}
    #print(f['market_attribute'][0][0][0])
...
def import_Market1501Attribute_binary(dataset_dir):
    ...
    label.insert(3,'old')
    testfile = './dataset/Market-1501/test_attr_label.txt'
    label[23] = 'downred'
    #print(train_market_attr)
    for personid in train_market_attr:
        filename = './dataset/Market-1501/anno/train/' + str(personid) + '.txt'
        f_train = open(filename,'w')
        for index in range(0,len(train_market_attr[personid])):    
            if index not in {0,1,2,3,7}:        
                f_train.write(label[index]+":"+str(train_market_attr[personid][index]) + "\n")
            if index == 20:
                f_train.write('upbrown'+":"+str(-1) + "\n")
        f_train.close()
    for personid in test_market_attr:
        filename = './dataset/Market-1501/anno/test/' + str(personid) + '.txt'
        f_test = open(filename,'w')
        for index in range(0,len(test_market_attr[personid])):      
            if index not in {0,1,2,3,7}:        
                f_test.write(label[index]+":"+str(test_market_attr[personid][index]) + "\n")
            if index == 20:
                f_test.write('upbrown'+":"+str(-1) + "\n")                    
        f_test.close()
...
def import_Market1501Attribute_binary(dataset_dir):
    ...
    #print(train_market_attr)
    filename = './dataset/all/anno/market_train.txt'
    f_train = open(filename,'w')
    for personid in train_market_attr:
        attrs = []
        upcolor = 0
        ...
        for index in range(0,len(train_market_attr[personid])):    
            if index not in {0,1,2,3,7}:  
                attrs.append(label[index] + ' '+ str(train_market_attr[personid][index]))
            if index > 12 and index < 21:
                upcolor = upcolor + train_market_attr[personid][index]
            if index > 20:
                downcolor = downcolor + train_market_attr[personid][index]
        if upcolor == 0:
            attrs.insert(16,'upothter 1')
        else:
            attrs.insert(16,'upothter 0')
        attrs.insert(16,'upbrown 0')
        if downcolor == 0:
        ...
        f_train.write(personid+":"+strvalues + "\n")
            ...
            #     f_train.write('upbrown'+":"+str(-1) + "\n")
    f_train.close()
    filename = './dataset/all/anno/market_test.txt'
    f_test = open(filename,'w')
    for personid in test_market_attr:
        attrs = []
        upcolor = 0
        ...
        for index in range(0,len(test_market_attr[personid])):      
            if index not in {0,1,2,3,7}:        
                attrs.append(label[index] + ' '+ str(test_market_attr[personid][index]))
            if index > 12 and index < 21:
                upcolor = upcolor + test_market_attr[personid][index]
            if index > 20:
                downcolor = downcolor + test_market_attr[personid][index]
        if upcolor == 0:
            attrs.insert(16,'upothter 1')
        else:
            attrs.insert(16,'upothter 0')
    ...
    label.insert(21,'upother')
    label.insert(21,'upbrown')
    label.append('downother')    
    filename = './dataset/all/anno/market_label.txt'
    f_label = open(filename,'w')
    for inx in range(0,len(label)):
        if inx not in {0,1,2,3,7}:  
            f_label.write(label[inx]+"\n")

            #print(train_market_attr[personid])
    return train_market_attr, test_market_attr, label
